...
$working = Join-Path $projectDir 'working_copy'
...
if (Test-Path $working) {
    $bak = "$working.bak-$(Timestamp)"
    Write-Host ("Existing working_copy found - renaming to: {0}" -f $bak)
    Rename-Item -Path $working -NewName (Split-Path $bak -Leaf)
}
