...
def test_per_controller_timeout():
    ...
    with app.app_context():
        ...
        Controller.query.filter(
            Controller.serial_number.like("TEST_TIMEOUT_%")
        ).delete()
        db.session.commit()
        ...
        test_controller_custom = Controller(
            ...
            - timedelta(seconds=90),  # 90 seconds ago (should be offline)
        )
        ...
        test_controller_default = Controller(
            ...
            last_seen=datetime.utcnow()
            - timedelta(seconds=90),  # 90 seconds ago (should still be online)
        )
        ...
        print(
            f"    Should be stale: {test_controller_custom.is_stale(Config.CONTROLLER_OFFLINE_TIMEOUT)}"
        )
        ...
        print(
            f"    Should be stale: {test_controller_default.is_stale(Config.CONTROLLER_OFFLINE_TIMEOUT)}"
        )
        ...
        print(f"\nAfter status check:")
        print(
            f"  Custom timeout controller is online: {test_controller_custom.is_online}"
        )
        print(
            f"  Default timeout controller is online: {test_controller_default.is_online}"
        )

        # Verify results
        assert (
            not test_controller_custom.is_online
        ), "Controller with custom 60s timeout should be offline after 90s"
        assert (
            test_controller_default.is_online
        ), "Controller with default 300s timeout should still be online after 90s"
...
def test_api_register_with_timeout():
    ...
    with app.test_client() as client:
        ...
        # Verify in database
        with app.app_context():
            controller = Controller.query.filter_by(
                serial_number="TEST_API_TIMEOUT_001"
            ).first()
            assert controller is not None
            assert controller.timeout_seconds == 120
            print(
                f"✅ Controller registered with timeout_seconds: {controller.timeout_seconds}"
            )
...
def test_api_modify_timeout():
    ...
    with app.test_client() as client:
        ...
        modify_data = {"timeout_seconds": 180}

        response = client.put(
            f"/api/controllers/{controller_id}",
